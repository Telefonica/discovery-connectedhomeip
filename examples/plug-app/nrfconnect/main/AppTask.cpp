...
CHIP_ERROR AppTask::Init()
{
    ...
    mPWMDevice.SetCallbacksEndpoints(ActionInitiated, ActionCompleted); 
...
void AppTask::ActionInitiated(PWMDevice::Action_t aAction, int32_t aActor, chip::EndpointId endpointId) 
{
    
    if (aAction == PWMDevice::ON_ACTION)
    {
        LOG_INF("Turn On Action has been initiated with endpoint : %d", endpointId);
    }
    else if (aAction == PWMDevice::OFF_ACTION)
    {
        LOG_INF("Turn Off Action has been initiated with endpoint : %d", endpointId);
    }
}

void AppTask::ActionCompleted(PWMDevice::Action_t aAction, int32_t aActor, chip::EndpointId endpointId)
{
    switch (endpointId){
        case 1:
        
            if (aAction == PWMDevice::ON_ACTION){
                LOG_INF("Turn On Action has been completed with endpoint : %d", endpointId);
                nrf_gpio_pin_set(42);
            }
            else if (aAction == PWMDevice::OFF_ACTION){
                LOG_INF("Turn Off Action has been completed with endpoint : %d", endpointId);
